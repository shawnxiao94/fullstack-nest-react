...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const pv = await this.loginService.getRedisPasswordVersionById(request[ADMIN_USER].id)
    if (pv !== `${request[ADMIN_USER].pv}`) {
      // 密码版本不一致，登录期间已更改过密码
      throw new ApiException(11002)
    }
    const redisToken = await this.loginService.getRedisTokenById(request[ADMIN_USER].id)
    if (token !== redisToken) {
      // 与redis保存不一致
      throw new ApiException(11002)
    }
