		...
		>
			<el-scrollbar class="layout-breadcrumb-seting-bar">
				...
				<div class="copy-config">
					<el-alert :title="$t('message.layout.tipText')" type="warning" :closable="false">
					</el-alert>
					<el-button
						size="default"
...
const onBgColorPickerChange = (bg: string) => {
	document.documentElement.style.setProperty(`--next-bg-${bg}`, (themeConfig as any).value[bg]);
	if (bg === 'menuBar') {
		document.documentElement.style.setProperty(
...
const setGraduaFun = (el: string, bool: boolean, color: string) => {
	setTimeout(() => {
		...
		if (bool)
			els.setAttribute(
				'style',
				`background:linear-gradient(to bottom left , ${color}, ${getLightColor(
					color,
					0.6
				)}) !important;`
			);
